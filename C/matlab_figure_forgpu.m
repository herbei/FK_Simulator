...
for i=1:NOBS
	SS(i,1) = SS(i,1)/(111e3 * cos(lat0*pi/180.0));	
	SS(i,2) = SS(i,2)/(111e3);
end
...
for j=1:NOBS
	idx = ceil( (nc-1)* ( DD(j) - p1 )/(p2-p1) );
	m_plot(SS(j,1), SS(j,2), '.', 'markersize', 50, 'color', cm(idx,:));
end
